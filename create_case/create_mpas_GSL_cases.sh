...
case_base="/scratch4/BMC/wrfruc/Michael.Barlage/mpas/baselines_mpas/create_case/gsl-v8.3.0-1.0-intelmpi/"
executable="/scratch4/BMC/wrfruc/Michael.Barlage/mpas/code/gsl/gsl-fork/MPAS-Model/init_atmosphere_model.v8.3.0-1.0.intelmpi"
...
if [ $partition = "xjet" ]; then 
  ...
  sbatch --account=$account --qos=$queue --partition=$partition --ntasks=8  --time=0:06:00 create_mpas_testcase_case03.sh $case_base $executable
  sbatch --account=$account --qos=$queue --partition=$partition --ntasks=24 --time=0:20:00 create_mpas_testcase_case04.sh $case_base $executable
  sbatch --account=$account --qos=$queue --partition=$partition --ntasks=8  --time=0:06:00 create_mpas_testcase_case05.sh $case_base $executable
  sbatch --account=$account --qos=$queue --partition=$partition --ntasks=24 --time=0:20:00 create_mpas_testcase_case06.sh $case_base $executable
  sbatch --account=$account --qos=$queue --partition=$partition --ntasks=8  --time=0:06:00 create_mpas_testcase_case07.sh $case_base $executable
  sbatch --account=$account --qos=$queue --partition=$partition --ntasks=8  --time=0:06:00 create_mpas_testcase_case08.sh $case_base $executable
fi
