...
#SBATCH --job-name=MPAS-C8-restart
#
...
if [ $clean_before = "true" ]; then 
  run_directory_pt1=$model_code_base.$physics_suite.restart1.$input_case_directory
  run_directory_pt2=$model_code_base.$physics_suite.restart2.$input_case_directory
fi
...
if [ $clean_before = "true" ]; then 
  if [ -d $run_directory_pt1 ]; then 
    echo
    echo "directory $run_directory_pt1 exists is being removed"
    rm -Rf $run_directory_pt1
  fi
  if [ -d $run_directory_pt2 ]; then 
    echo
    echo "directory $run_directory_pt2 exists is being removed"
    rm -Rf $run_directory_pt2
  fi
fi

if [ ! -d $run_directory_pt1 ]; then 
  mkdir -p $run_directory_pt1
fi
cd $run_directory_pt1

echo
echo "################################################################"
echo "# Start setting up case directory for cold start of restart comparison"
echo "################################################################"
...
cp $script_home/../run_case/case_files/$namelist_version/$domain/$source.$yyyy$mm$dd$hh/$physics_suite.restart1/* .
...
if [ ! -d $run_directory_pt2 ]; then 
  mkdir -p $run_directory_pt2
fi
cd $run_directory_pt2

echo
echo "################################################################"
echo "# Start setting up case directory for restart of restart comparison"
echo "################################################################"
...
ln -sf ../$run_directory_pt1/restart.* .
...
cp $script_home/../run_case/case_files/$namelist_version/$domain/$source.$yyyy$mm$dd$hh/$physics_suite.restart2/* .
...
echo
echo "################################################################"
echo "# Successful completion of restart case: $model_code_base.$physics_suite.$input_case_directory"
echo "################################################################"
